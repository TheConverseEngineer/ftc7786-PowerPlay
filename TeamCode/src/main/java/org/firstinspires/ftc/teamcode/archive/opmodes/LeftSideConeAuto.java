...
@Config
@Autonomous(name = "left side (cone)")
public class LeftSideConeAuto extends CommandOpMode {

    // Five inches from outer side to inner edge of mat

    public static double ARM_DROP = 510;
    public static double END_Y = 62, END_X = 0;
    ...
    public void init(CommandScheduler master) {
        ...
        TrajectorySequence right =  drive.trajectoryBuilder(new Pose2d(END_X, END_Y-11, 0), 0)
                .splineToLinearHeading(new Pose2d(END_X+21, END_Y - 11, 0), 0).build();

        TrajectorySequence left =  drive.trajectoryBuilder(new Pose2d(END_X, END_Y-14, 0), Math.PI)
                .splineToLinearHeading(new Pose2d(END_X-21, END_Y - 14, 0), Math.PI).build();
        ...
        master.ScheduleCommand(
                ...
                new SequentialCommand(
                        ...
                        new InstantCommand(() -> armServo.setPosition(0.4)))
        );
    }
